...
task SortSam {
  ...
  Float sort_sam_disk_multiplier = 7.25
  Int disk_size = ceil(sort_sam_disk_multiplier * size(input_bam, "GiB")) + 20

  command {
    java -Dsamjdk.compression_level=~{compression_level} -Xms15000m -jar /usr/gitc/picard.jar \
      SortSam \
      INPUT=~{input_bam} \
...
task MarkDuplicates {
  ...
  Float memory_size = 20.5 * memory_multiplier
...
task GenerateSubsettedContaminationResources {
  ...
  runtime {
    ...
    docker: "gcr.io/arcus-jpe-pipe-stage-4f4279cc/bedtools:2.27.1"
  }
  output {
